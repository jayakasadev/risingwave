...
impl CatalogController<Client, ClientSession> {
    /// Fill in the `cdc_table_id` field for Table with empty `cdc_table_id` and parent Source job.
    /// NOTES: We assume Table with a parent Source job is a CDC table
    pub(crate) async fn table_catalog_cdc_table_id_update(&self) -> MetaResult<()> {
        let inner = self.inner.read().await;
    ...
    pub(crate) async fn list_object_dependencies(
        ...
        let dependencies: Vec<(ObjectId, ObjectId)> = {
            ...
            let aggr= vec!{
                // project
                doc!{"$project": {"oid": "$oid", "used_by": "$used_by"}},
                // self join to get dependency chain
                doc!{"$lookup": {"from": "object", "localField": "used_by", "foreignField": "oid", "as": "dependencies"}},
                doc!{"$unwind": "$dependencies", "preserveNullAndEmptyArrays": false }, // filter anything without a join value
                doc!{"$lookup": {"from": "streaming_job", "localField": "oid", "foreignField": "owner_id", "as": "jobs"}},
                doc!{"$unwind": "$jobs", "preserveNullAndEmptyArrays": false }, // filter anything without a join value

                if !include_creating {
                    doc!{}
                } else {
                    doc!{"job_status": JobStatus::Created}
                }
            };
            &inner.db
                .default_database()
                .unwrap()
                .collection("object_dependency")
                .aggregate(aggr)
                .await?;
            ObjectDependency::find()
                .select_only()
