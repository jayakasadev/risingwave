...
use sea_orm::{ConnectionTrait, DatabaseBackend, DatabaseConnection, FromQueryResult, Statement, TransactionTrait, Value};
...
impl Driver for MongoDBDriver {
    ...
    async fn try_campaign(
        ...
        let query_result: Option<Document> = self.conn
            .default_database()
            .unwrap()
        ...
        let row = query_result
            .map(|query_result: Document| ElectionRow{
